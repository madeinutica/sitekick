...
- [x] Integrate MarketSharp OData API to pull customer information and jobs daily
  - **COMPLETED**: Built full MarketSharp integration using their OData read-only API with HMAC-SHA256 authentication. Created API client library (src/lib/marketsharp/client.ts) with typed endpoints for Contacts, Jobs, Inquiries, Appointments, Employees, and Contracts. Created sync engine (src/lib/marketsharp/sync.ts) that upserts contacts/jobs into mirror tables and auto-creates linked jobs in the main jobs table. Added API routes for test connection, manual sync, contact browsing, and a daily cron endpoint. Created admin UI page at /admin/marketsharp with overview dashboard, contacts/jobs browser, and sync log viewer. Added database migration (marketsharp_migration.sql) with ms_contacts, ms_jobs, ms_sync_log tables and RLS policies. Configured Vercel cron (vercel.json) for daily sync at 6 AM UTC. Application compiles successfully.
