...
export default function JobDetailPage() {
  ...
  // ...other hooks and logic as needed...

  return (
    <main>
      {/* Job Comments Section - Styled Card (replaces Notes card) */}
      <div className="mt-10">
        <div className="bg-white rounded-xl border border-slate-200 p-8 shadow-sm max-w-2xl mx-auto mb-8">
              ...
              comments.map(comment => (
                <div key={comment.id} className="flex items-start gap-3">
                  {comment.user?.avatar_url ? (
                    <Image src={comment.user.avatar_url} alt="avatar" width={40} height={40} className="rounded-full w-10 h-10 object-cover" />
                  ) : (
                  ...
                  <div className="flex-1">
                    <div className="font-bold text-slate-900 leading-tight">{comment.user?.full_name || 'User'}</div>
                    <div className="text-slate-800 mb-1 whitespace-pre-line">{comment.content}</div>
                    <div className="text-xs text-slate-500">{formatTimeAgo(comment.created_at)}</div>
                  </div>
                </div>
              ))
            ) : (
          ...
          <form className="flex items-center gap-3 pt-2 border-t border-slate-100" onSubmit={e => { e.preventDefault(); handlePostComment(); }}>
            {profile?.avatar_url ? (
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      ...
      if (!error && data) {
        // Build thread tree
        const byId: { [id: string]: JobComment } = {}
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-indigo-50 via-white to-blue-50">
      ...
      {/* Job Comments Section - Styled Card (moved outside photos card) */}
      <div className="mt-10">
        <div className="bg-white rounded-xl border border-slate-200 p-8 shadow-sm max-w-2xl mx-auto mb-8">
          <h2 className="text-lg font-semibold text-slate-900 mb-4">Comments</h2>
          <div className="space-y-5 mb-4">
